...
int main()
{
    int NumTree, LengthTree;
    cin >> NumTree >> LengthTree;
    vector<int> trees;
    int highestTree = 0;
    for (int i = 0; i < NumTree; i++)
    {
        ...
        if (tree > highestTree) highestTree = tree;

    }
    int low = 0;
    int high = highestTree;
    int mid = 0;
    ...
    while (low <= high)
    {
        ...
        mid = (low + high) / 2;
        for(int i = 0; i < NumTree; i++)
            cutTreeSum += max(trees[i] - mid,0);

        if (cutTreeSum >= LengthTree)
        { //톱길이를(mid) 높혀야 한다.
            low = mid + 1;
            ans = mid;
        }
